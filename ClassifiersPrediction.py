...
def main(argv=None):
    ...
    #language_model = gensim.models.KeyedVectors.load_word2vec_format('../../models/GoogleNews-vectors-negative300.bin', binary=True)
    ...
    #embedding, goldlabels = extract_embeddings_as_features_and_gold(inputfile,language_model)
    
    #we can use the same function as before for creating the classifier and vectorizer
    #if all of the system output files are needed one can run the code as is. However, if you only want to check the feature ablation for SVM (the system I used), all of the other systems mentioned as 'if modelname == ...' can be disgarded by putting a # in front of it. Also in order to not create a new SVM output file all the time, one can also put a # in front of the classify_data function call under 'if modelname == 'SVM''.
    for modelname in ['logreg', 'NB', 'SVM', 'Bernoulli']:
